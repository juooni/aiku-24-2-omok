...
from model1 import Residual_CNN ###model -> model1
...
def playMatches(player1, player2, EPISODES, logger, turns_until_tau0, memory = None, goes_first = 0):
    ...
    for e in range(EPISODES):
        ...
        while done == 0:
            ...
            print('')
            env.gameState.printBoard() # for debugging

    return (scores, memory, points, sp_scores)


def UserplayMatches(player1, player2, EPISODES, logger, turns_until_tau0, memory = None, goes_first = 0):
    # player1을 user로 하자자
    env = Game()
    scores = {player1.name:0, "drawn": 0, player2.name:0}
    sp_scores = {'sp':0, "drawn": 0, 'nsp':0}
